        ...
        <main class="main-content" id="mainContent">
            ...
            <div id="settings-tab" class="tab-content">
                <div class="content-area">
                    ...
                    <div class="settings-section">
                        <h3>Web Interface Authentication</h3> <!-- NEW SECTION -->
                        <div class="form-group">
...
function renderSettings() {
    ...
    document.getElementById('web-admin-username').value = appSettings.webAdminUsername || ''; // NEW
    document.getElementById('web-admin-password').value = appSettings.webAdminPassword || ''; // NEW
    ...
    document.getElementById('setting-random-mac').checked = appSettings.enableRandomMac || false; // NEW
    document.getElementById('setting-channel-hop').checked = appSettings.enableChannelHopping || false; // NEW
}
...
async function saveSettings() {
    const newSettings = {
        ...
        webAdminUsername: document.getElementById('web-admin-username').value, // NEW
        webAdminPassword: document.getElementById('web-admin-password').value, // NEW
        ...
        enableRandomMac: document.getElementById('setting-random-mac').checked, // NEW
        enableChannelHopping: document.getElementById('setting-channel-hop').checked // NEW
    };
...
void setup() {
  ...
  webServer.on("/upload", HTTP_POST, []() {
    if (!isAuthenticated()) return webServer.requestAuthentication();
    handleFileUpload();
  }); // Corrected: only pass the handler function
...
void loop() {
  ...
  if (deauthing_active && _selectedNetwork.ssid != "" && millis() - lastDeauthTime >= 1000) {
    ...
    // --- Tambahan: Informasi Paket yang Dikirim ---
...
void loadSettings() {
  ...
  appSettings.webAdminUsername = "admin"; // NEW default username
  appSettings.webAdminPassword = "password"; // NEW default password
  ...
  if (SPIFFS.exists(SETTINGS_FILE)) {
    ...
    if (settingsFile) {
      ...
      if (!error) {
        ...
        appSettings.webAdminUsername = doc["webAdminUsername"] | appSettings.webAdminUsername; // NEW
        appSettings.webAdminPassword = doc["webAdminPassword"] | appSettings.webAdminPassword; // NEW
...
void saveSettings() {
  ...
  if (settingsFile) {
    ...
    doc["webAdminUsername"] = appSettings.webAdminUsername; // NEW
    doc["webAdminPassword"] = appSettings.webAdminPassword; // NEW
...
void handleApiSaveSettings() {
  if (webServer.hasArg("plain")) {
    ...
    String newWebAdminUsername = doc["webAdminUsername"] | appSettings.webAdminUsername; // NEW
    String newWebAdminPassword = doc["webAdminPassword"] | appSettings.webAdminPassword; // NEW

    bool apSettingsChanged = (newAdminApSsid != appSettings.adminApSsid || newAdminApPassword != appSettings.adminApPassword);
    bool webAuthSettingsChanged = (newWebAdminUsername != appSettings.webAdminUsername || newWebAdminPassword != appSettings.webAdminPassword); // NEW
    ...
    appSettings.webAdminUsername = newWebAdminUsername; // NEW
    appSettings.webAdminPassword = newWebAdminPassword; // NEW
    ...
    if (apSettingsChanged || webAuthSettingsChanged) { // Restart if web auth changes too
