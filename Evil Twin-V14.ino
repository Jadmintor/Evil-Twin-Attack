...
void updateOLEDDisplay() {
  ...
  // Baris 0: APP_NAME & Mode
  display.setCursor(0, 0);
  display.print(APP_NAME);
  display.setCursor(SCREEN_WIDTH - (oledModeNames[appSettings.oledDisplayMode].length() * 6), 0); // 6 pixels per char
  display.print(oledModeNames[appSettings.oledDisplayMode]);
  display.drawFastHLine(0, 8, SCREEN_WIDTH, SSD1306_WHITE); // Garis di bawah judul

  // --- Hapus informasi AP dan STA ---

  // --- NEW/MODIFIED FEATURE: Customizable Display Modes ---
  switch (appSettings.oledDisplayMode) {
    case MODE_IDLE_INFO: {
      // Baris 1: Jumlah Klien & Status Serangan Utama
      ...
      // Baris 2: Status Target Jaringan
      ...
      // Baris 3: Free Heap & Memory Bar
      display.setCursor(0, 36);
      display.print("Free Heap: ");
      display.print(ESP.getFreeHeap() / 1024);
      display.print("KB");

      // --- Graphical Indicators (Memory Bar) ---
      ...
      int barWidth = map(memoryUsagePercent, 0, 100, 0, SCREEN_WIDTH - 50); // Bar width, leaving space for text
      display.drawRect(0, 46, SCREEN_WIDTH, 10, SSD1306_WHITE); // Outline
      display.fillRect(1, 47, barWidth - 2, 8, SSD1306_WHITE); // Fill
      display.setCursor(SCREEN_WIDTH - 45, 36);
    ...
    case MODE_ATTACK_STATS: {
      ...
      if (deauthing_active) {
        ...
        display.print("No Attack Active");
      }

      // Baris 2: Paket Deauth / Kredensial Tertangkap
      display.setCursor(0, 24);
      if (deauthing_active) {
      ...
      // Baris 3: Kredensial Terakhir
      display.setCursor(0, 36);
      if (hotspot_active) {
        display.print("Last Cred: ");
        String lastCred = lastCapturedCredential;
        if (lastCred.length() > 10) lastCred = lastCred.substring(0, 10) + "...";
        display.print(lastCred);
      } else if (deauthing_active) {
        display.print("Target: ");
        String targetSsid = _selectedNetwork.hidden ? "(Hidden)" : _selectedNetwork.ssid;
        if (targetSsid.length() > 10) targetSsid = targetSsid.substring(0, 10) + "...";
        display.print(targetSsid);
      }
      break;
    }
    ...
    case MODE_NETWORK_SCAN: {
      ...
      // Baris 2-4: List 3 jaringan teratas
      for (int i = 0; i < 3; ++i) {
        if (_networks[i].ssid != "" || _networks[i].hidden) {
          ...
          if (ssidDisplay.length() > 12) ssidDisplay = ssidDisplay.substring(0, 12) + "...";
          display.print(ssidDisplay);
          display.print(" (");
          display.print(_networks[i].rssi);
          display.print(")");
          // Bar sinyal
          int signalBarWidth = map(_networks[i].rssi, -100, -30, 0, 20);
          signalBarWidth = constrain(signalBarWidth, 0, 20);
          display.drawRect(SCREEN_WIDTH - 25, 24 + (i * 10), 25, 8, SSD1306_WHITE);
          display.fillRect(SCREEN_WIDTH - 24, 25 + (i * 10), signalBarWidth, 6, SSD1306_WHITE);
        }
      }
      break;
    }
