...
// unsigned long lastScanTime = 0; // Dihapus: Tidak lagi diperlukan untuk scan otomatis
...
void handleApiManualScan(); // NEW: Prototype for manual scan API
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // scanNetworks(); // Dihapus: Tidak lagi scan otomatis saat startup
    fetchSettings();
...
function attachEventListeners() {
    ...
    document.getElementById('quick-scan-btn').addEventListener('click', () => {
        showTab('scanner');
        // scanNetworks(); // Dihapus: Quick scan sekarang memicu manual scan
        manualScan(); // Panggil fungsi manual scan
    });
    document.getElementById('generate-report-btn').addEventListener('click', generateReport);

    // document.getElementById('refresh-scan-btn').addEventListener('click', scanNetworks); // Dihapus: Diganti dengan manual-scan-btn
    document.getElementById('manual-scan-btn').addEventListener('click', manualScan); // NEW: Event listener untuk tombol manual scan
...
async function scanNetworks() {
    // addToTerminal('[SCAN] Starting WiFi network scan...'); // Ini akan dipanggil setelah manual_scan
    try {
...
void setup() {
  ...
  webServer.on("/api/manual_scan", HTTP_GET, []() { // NEW: Endpoint untuk memicu scan manual
    if (!isAuthenticated()) return webServer.requestAuthentication();
    handleApiManualScan();
  });
  ...
  // performScan(); // Dihapus: Tidak lagi scan otomatis saat startup
  startTime = millis();
  updateOLEDDisplay();
}
...
void loop() {
  ...
  // Dihapus: Tidak lagi scan otomatis
...
void handleApiManualScan() { // NEW: Handler untuk memicu scan manual
  performScan(); // Lakukan pemindaian
  webServer.send(200, "application/json", "{\"success\": true, \"message\": \"Scan initiated\"}");
}

void handleApiScan() {
  // Fungsi ini hanya akan mengembalikan data scan yang sudah ada di _networks
